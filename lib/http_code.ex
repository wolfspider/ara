defmodule HttpCode do
  ...
  # Convert integers to atoms if possible
  defp atom_for_code(code) when is_integer(code) do
    Enum.find(@http_codes, fn {_key, val} -> val == code end)
    |> case do
      {key, _val} -> key
      nil -> nil
    end
  ...
  defp atom_for_code(code) when is_atom(code), do: code
  defp atom_for_code(_), do: nil

  def code(http_code) do
    http_code
    |> atom_for_code()
    |> (&Map.get(@http_codes, &1, :unknown_code)).()
  end

  def status(http_code) do
    http_code
    |> atom_for_code()
    |> (&Map.get(@http_status, &1, "Unknown status")).()
  end

  def message(http_code) do
    http_code
    |> atom_for_code()
    |> (&Map.get(@http_message, &1, "Unknown message")).()
  end
end
